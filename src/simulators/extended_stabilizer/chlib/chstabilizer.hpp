...
public:
  ...
  uint_fast64_t NQubits() const { return n; }
  scalar_t Omega() const { return omega; }
  uint_fast64_t Gamma1() const { return gamma1; }
  uint_fast64_t Gamma2() const { return gamma2; }
  std::vector<uint_fast64_t> GMatrix() const { return G; }
  std::vector<uint_fast64_t> FMatrix() const { return F; }
  std::vector<uint_fast64_t> MMatrix() const { return M; }
  ...
  scalar_t InnerProduct(const uint_fast64_t &A_diag1,
...
StabilizerState::StabilizerState(const unsigned n_qubits)
    : n(n_qubits), // number of qubits
      gamma1(zer), gamma2(zer), F(n, zer), G(n, zer), M(n, zer), v(zer), s(zer),
      omega(), FT(n, zer), MT(n, zer), isReadyFT(false), isReadyMT(false) {
  // initialize  by the basis vector 00...0
  if (n > 63) {
    throw std::invalid_argument(
        "The CH simulator only supports up to 63 qubits.\n");
  }
...
StabilizerState::StabilizerState(const StabilizerState &rhs)
    : n(rhs.n), // number of qubits
      gamma1(rhs.gamma1), gamma2(rhs.gamma2), F(rhs.F), G(rhs.G), M(rhs.M),
      v(rhs.v), s(rhs.s), omega(rhs.omega), FT(rhs.FT), MT(rhs.MT),
      isReadyFT(rhs.isReadyFT), isReadyMT(rhs.isReadyMT) {}
...
scalar_t StabilizerState::Amplitude(uint_fast64_t x) {
  ...
  if (!omega.eps)
    return omega; // the state is zero
  ...
  for (unsigned q = 0; q < n; q++) {
    ...
    if (v & pos) {
      amp.e +=
          4 * ((s & pos) && (P.X & pos)); // minus sign that comes from <1|H|1>
    } else {
...
scalar_t StabilizerState::ProposeFlip(unsigned flip_pos) {
  ...
  for (unsigned q = 0; q < n; q++) {
    ...
    if (v & pos) {
      amp.e +=
          4 * ((s & pos) && (Q.X & pos)); // minus sign that comes from <1|H|1>
    } else {
...
void StabilizerState::UpdateSvector(uint_fast64_t t, uint_fast64_t u,
                                    unsigned b) {
...
void StabilizerState::H(unsigned q) {
  ...
  unsigned beta =
      AER::Utils::popcount((rowM & (~v) & s) ^ (rowF & v & (rowM ^ s)));
...
void StabilizerState::MeasurePauli(pauli_t PP) {
  ...
  for (unsigned j = 0; j < n; j++)
    if ((PP.X >> j) & one) {
      ...
      R.e += 2 * AER::Utils::popcount(
                     R.Z & rowF); // extra sign from Pauli commutation
      R.Z ^= rowM;
      R.X ^= rowF;
...
scalar_t StabilizerState::InnerProduct(const uint_fast64_t &A_diag1,
                                       const uint_fast64_t &A_diag2,
                                       const std::vector<uint_fast64_t> &A) {
  uint_fast64_t K_diag1 = zer, K_diag2 = zer, J_diag1 = gamma1,
                J_diag2 = gamma2;
  ...
  for (size_t i = 0; i < n; i++) {
    if ((v >> i) & one) {
      ...
      q.D2 ^= ((((K_diag2 >> i) ^ (s >> i)) & one) ^
               AER::Utils::hamming_parity(K[i] & s)) *
              shift;
      // q.D2 ^= ((s >> i) & one) * shift;
  ...
  for (size_t i = 0; i < n; i++) {
    if ((s >> i) & one) {
      q.Q = (q.Q + 4 * ((K_diag2 >> i) & one) + 2 * ((K_diag1 >> i) & one)) %
            8; // + 2*((K_diag1 >> i) & one))%8;
